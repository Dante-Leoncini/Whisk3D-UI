...
GroupPropertie::GroupPropertie(const std::string& Name): name(Name), icono(-1) {
    ...
    editando = false;
    anchoValores = 0.5f;
    reservaKeyBtn = false; keyBtnCol = 0;
    colEtiqueta = 150;
    width = 300;
    height = 300;
    maxPixelsTitle = 1920;
...
void GroupPropertie::Resize(int Width, int Height){
    ...
    // columna del boton de KEYFRAME (a la derecha de todo): un cuadrado + gap
    keyBtnCol = reservaKeyBtn ? (RenglonHeightGS + GlobalScale*2 + gapGS) : 0;
    anchoBox -= keyBtnCol;
    colEtiqueta = widthProperties - anchoBox - keyBtnCol;
...
void GroupPropertie::RenderPropertiBox(){
    ...
    for (size_t i = 0; i < properties.size(); ++i){
        ...
        // BOTON de keyframe COMPLETO (fondo + borde + rombo) a la DERECHA de todo.
        // OJO: RenderPropertiBox ya avanzo la Y una fila -> back-translate para
        // volver a la fila de ESTA propiedad. El rombo se centra en el cuadrado.
        if (reservaKeyBtn && properties[i]->AnimProp() >= 0){
            int est = W3dKeyframeEstado ? W3dKeyframeEstado(properties[i]->AnimProp(), properties[i]->AnimComp()) : 0;
            int btnLado = checkBox->width;                 // el cuadrado (= el del checkbox)
            bool foco = (g_kfFocoProp == (void*)properties[i]);   // enfocado por TECLADO
            w3dEngine::PushMatrix();
            w3dEngine::Translatef((GLfloat)(propertiBox->width + gapGS), (GLfloat)(-(RenglonHeightGS + gapGS)), 0);
            // SIN recuadro por defecto (solo el icono). El BORDE BLANCO se dibuja
            // unicamente cuando se navega con las flechas (Symbian/Nokia): asi sabes
            // que estas pisando el keyframe y con OK/Enter lo pones/sacas.
            if (foco){ SetColorID(ColorID::blanco, 1.0f); checkBox->RenderBorder(false); }
            w3dEngine::Translatef((GLfloat)((btnLado - IconSizeGS)/2), (GLfloat)((btnLado - IconSizeGS)/2), 0);
            W3dRenderRomboKey(est);
            w3dEngine::PopMatrix();
            SetColorID(ColorID::background, 1.0f);
        }
...
void GroupPropertie::RenderPropertiValue(){
    PropColEtiqueta = colEtiqueta; g_propKeyBtnCol = keyBtnCol;
