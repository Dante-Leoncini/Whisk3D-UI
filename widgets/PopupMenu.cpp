...
// ---- DRAG del item-slider (AgregarFloat) ----
// El DOWN sobre la fila de un slider lo ARMA (dentro de Click, que ademas setea el
// valor como siempre -> un click corto sigue siendo click). Mientras el boton siga
// apretado, la app rutea cada motion a MenuSliderDragMover: la X del mouse se
// re-mapea a la barra (mismo mapeo que el click) y el valor cambia EN VIVO.
// El menu dueno de la fila puede ser un SUBMENU: se guarda el menu que la contiene.
static PopupMenu* g_sliderDragMenu = NULL; // menu dueno de la fila arrastrada (NULL = no hay drag)
static MenuItem*  g_sliderDragItem = NULL;

bool MenuSliderDragActivo(){
    return g_sliderDragMenu != NULL && g_sliderDragMenu->abierto &&
           g_sliderDragItem != NULL;
}

int MenuSliderDragMover(int mx){
    if (!MenuSliderDragActivo()) return -1;
    MenuItem* it = g_sliderDragItem;
    if (!it->valorFloat) return -1;
    if (it->gris && !*it->gris) return -1; // se deshabilito en el medio: no editar
    int sliderW = 80 * GlobalScale;        // mismo ancho/mapeo que Click y Render
    int barXscreen = g_sliderDragMenu->x + borderGS +
                     (g_sliderDragMenu->width - bordersGS - sliderW);
    float frac = (float)(mx - barXscreen) / (float)sliderW;
    if (frac < 0.0f) frac = 0.0f; if (frac > 1.0f) frac = 1.0f;
    *it->valorFloat = it->vmin + frac * (it->vmax - it->vmin);
    return it->id;
}

void MenuSliderDragSoltar(){
    g_sliderDragMenu = NULL;
    g_sliderDragItem = NULL;
}
...
PopupMenu::~PopupMenu(){
    if (g_sliderDragMenu == this) MenuSliderDragSoltar();
...
void PopupMenu::Limpiar(){
    if (g_sliderDragMenu == this) MenuSliderDragSoltar(); // los items se borran: no dejar un puntero colgado
    for (size_t i = 0; i < items.size(); i++) delete items[i];
    items.clear();
    selectIndex = -1;
}
...
void PopupMenu::Cerrar(){
    if (g_sliderDragMenu == this) MenuSliderDragSoltar(); // menu cerrado (ESC, etc): corta el drag
    abierto = false;
    selectIndex = -1;
    if (submenuAbierto){
...
void PopupMenu::Render(){
    ...
    if (!titulo.empty()){
        ...
        w3dEngine::PopMatrix();
        // LINEA separadora bajo el titulo: un gris MAS CLARO que el fondo del menu
        // (el fondo es 'gris' 0.12; el negro no se veia -> grisLinea 0x494949).
        {
            const float* ac = ListaColores[static_cast<int>(ColorID::grisLinea)];
...
int PopupMenu::Click(int mx, int my){
    ...
    if (itF->valorFloat && !(itF->gris && !*itF->gris)){
        ...
        *itF->valorFloat = itF->vmin + frac * (itF->vmax - itF->vmin);
        // ARMA el drag: si el boton sigue apretado, los motion siguientes re-mapean
        // la X a la barra y editan el valor en vivo (MenuSliderDragMover)
        g_sliderDragMenu = this; g_sliderDragItem = itF;
        return itF->id;
    }
    return Enter();
}
